...
public class Condominium extends AppCompatActivity {
    ...
    ArrayList<GetHouse> houseList = getCondos();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_condominium);
        RecyclerView recyclerView = findViewById(R.id.RecyclerCondo);
        HomeDetail adapter = new HomeDetail(getApplicationContext(),houseList);
    ...
    public ArrayList<GetHouse> getCondos(){
        ArrayList<GetHouse> condominium = new ArrayList<>();
        for(GetHouse home : GetHouse.getHomeList){
            if(home.getHomeType().equals("condominium")) condominium.add(home);
        }
        return condominium;
    }
}