...
public class SemiDetachedHome extends AppCompatActivity {
    ...
    ArrayList<GetHouse> homeList = getSemis();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_semi_detached_home);
        RecyclerView recyclerView = findViewById(R.id.RecyclerSemiDetached);
    ...
    public ArrayList<GetHouse> getSemis(){
        ArrayList<GetHouse> homes = new ArrayList<>();
        for(GetHouse home : GetHouse.getHomeList){
            if(home.getHomeType().equals("semi")) homes.add(home);
        }
        return homes;
    }
}