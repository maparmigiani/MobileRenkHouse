...
public class DetachedHome extends AppCompatActivity {
    ...
    ArrayList<GetHouse> homeList = getDetaches();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_detached_home);
        RecyclerView recyclerView = findViewById(R.id.RecyclerDetached);
    ...
    public ArrayList<GetHouse> getDetaches(){
        ArrayList<GetHouse> homes = new ArrayList<>();
        for(GetHouse home : GetHouse.getHomeList){
            if(home.getHomeType().equals("detached")) homes.add(home);
        }
        return homes;
    }
}